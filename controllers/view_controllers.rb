...
module TerminalController
  ...
  def self.preserve_session
    ...
    while MorningMusic.power_on
      ...
      command_focus.size == 1 ? command : command = command_focus[0]
      ...
      when "ADD" then TaskController.execute(command, command_focus[1..-1])
      when "A" then TaskController.execute(command, command_focus[1..-1])
      when "DELETE" then TaskController.execute(command, command_focus[1..-1])
      when "D" then TaskController.execute(command, command_focus[1..-1])
      else
        puts "I don't understand.. please tell me what to do!\n\n"
        sleep 3
      end
    end
  end
